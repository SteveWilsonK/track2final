"""Pillar 3a: the agent's structured memory (belief state).

Between sessions, the agent previously re-read markdown logs and re-derived
its understanding — notes, not memory. This module is a persistent JSON
belief state with a strict API, and one rule enforced as code rather than
as an instruction:

    promote() REFUSES to mark a mechanism-tagged hypothesis as confirmed
    unless a passing falsification control (controls.py) is recorded.
    Not a guideline. An exception.

Records: {id, claim, mechanism (temporal|capacity|objective|none), status
(proposed|testing|confirmed|refuted|refuted_by_control), evidence [run
results], control {type, outcome, detail}, expected_value, cost_estimate_s}.

Usage:
    python3 belief_state.py --selftest
    python3 belief_state.py --next        # highest-priority open hypothesis
    (as a library) propose / attach_evidence / attach_control / promote
"""
import json, os, sys
