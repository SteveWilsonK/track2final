...
1. `logs/RESULTS.md` — the full research narrative so far
2. `logs/IDEAS.md` — backlog: OPEN / DEAD / PARKED / BANKED, with reasons
3. `logs/LOG.jsonl` — machine log; the last entries are the freshest state
4. `code/harness.py` — the experiment API you must use
...
2. **Write** the experiment as a standalone script in
   `code/` following the pattern of run*.py
