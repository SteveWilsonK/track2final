"""Mechanism test for the causal-sequence-feature claim (Pillar 1 flagship).

The claim under interrogation: "the sequence features work because they
carry what-the-user-just-did information" (a temporal mechanism). This
...
three-way decomposition with retrained 5-seed committees:
  ...
  D  seq features replaced by matched-cardinality  -> pure added-capacity
     random tokens                                    control

Decomposition:
  total seq gain      = A - B
  timing component    = A - C   (lost when alignment breaks)
  fingerprint component = C - B (survives shuffling)
  capacity component  = D - B   (should be ~0; if not, embeddings alone help)

All models here are diagnostics, trained to be compared and discarded. The
designated submission is untouched. Runs are logged through the harness
for provenance (their verdicts are expected to be non-promotions).

Run from code/:  python3 mechanism_test.py     (~15 min)
"""
